package com.apple.mock

import java.util.UUID

import com.apple.commons.model.{ProductInfo, UserInfo, UserVisitAction}
import com.apple.commons.utils.{DateUtils, StringUtils}
...
object MockDataGenerate {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 创建Spark SQL 客户端
    val spark = SparkSession.builder().appName("MockData")
      .master("local[*]")
      //.config("spark.sql.warehouse.dir",warehouseLocation)
      .enableHiveSupport()
      .getOrCreate()

    //spark.sql("select * from user_visit_action limit 1").show()
