package com.apple.session

import java.util.{Date, UUID}

import com.apple.commons.conf.ConfigurationManager
import com.apple.commons.constant.Constants
import com.apple.commons.model.{UserInfo, UserVisitAction}
import com.apple.commons.utils._
...
/**
  * @Program: commerce
  * @ClassName: UserVisitSessionAnalyze
  * @Description: TODO
  * @Author Mr.Apple
...
object UserVisitSessionAnalyze {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 构建Spark上下文
    val sparkConf =
      new SparkConf().setAppName("SessionAnalyzer")
        .setMaster("local[*]")
    ...
    //        // 设置driver的ip地址
    //        .set("spark.driver.host", "192.168.230.1")
    //        // 设置jar包的路径,如果有其他的依赖包,可以在这里添加,逗号隔开
    //        .set("spark.yarn.jars", "hdfs://master:9000/spark-yarn-jars/jars/*.jar")
    //        //.set("spark.executor.extraJavaOptions", "-Xss4m")
    //        .setJars(List("D:\\IdeaProjects\\commerceDemo\\target\\commerceDemo-1.0-SNAPSHOT.jar"))
    ...
    //sessionid2detailRDD ( sessionId, userAction )
    //top10CategoryList   ((clickCount, orderCount, payCount), clickCount=10|orderCount=10|payCount=8)
    // 业务功能四：获取top10热门品类的活跃session

    /**
      * top10CategoryList(0): (CategorySortKey(23795,0,0),categoryid=-1|clickCount=23795|orderCount=0|payCount=0)
      *
      * sessionid2detailRDD 7b379256363b41ad8ab9a1a6e26dcf02	UserVisitAction(2021-10-12,87,7b379256363b41ad8ab9a1a6e26dcf02,9,2021-10-12 11:36:05,null,-1,-1,null,null,32,62,3)
      *
      */
  ...
  def getTop10Session(spark: SparkSession, taskid: String, top10CategoryList: Array[(CategorySortKey, String)], sessionid2ActionRDD: RDD[(String, UserVisitAction)]) = {
    ...
    /**
      * top10CategoryIdRDD (categoryid, categoryid)
      *
      * categoryid2sessionCountRDD  (categoryid, sessionid + "," + count)
      *
      * top10CategorySessionCountRDD  (categoryid  sessionid + "," + count)
      */

    // 通过top10热门品类top10CategoryIdRDD与完整品类点击统计categoryid2sessionCountRDD进行join，仅获取热门品类的数据信息
    // 获取到to10热门品类，被各个session点击的次数【将数据集缩小】
    val top10CategorySessionCountRDD = top10CategoryIdRDD.join(categoryid2sessionCountRDD).map { case (cid, (ccid, value)) => (ccid, value) }
  ...
  def getTop10Category(spark: SparkSession, taskid: String, sessionid2detailRDD: RDD[(String, UserVisitAction)]) = {
  ...
  def randomExtractSession(spark: SparkSession, taskUUID: String, sessionid2AggrInfoRDD: RDD[(String, String)], sessionid2actionRDD: RDD[(String, UserVisitAction)]) = {
    ...
    // 第二步，使用按时间比例随机抽取算法，
    // 计算出每天每小时要抽取session的索引，
    // 将<yyyy-MM-dd_HH,count>格式的map，
    // 转换成<yyyy-MM-dd,<HH,count>>的格式
    ...
    val sessionRandomExtract = time2sessionsRDD.flatMap { case (dateHour, items) =>
      ...
      // 开始遍历所有的aggrInfo
      for (sessionAggrInfo <- items) { //item ： aggrInfo
  ...
  def aggregateBySession(spark: SparkSession, sessionid2actionRDD: RDD[(String, UserVisitAction)]) = {
    ...
    import spark.implicits._
    val userid2InfoRDD = spark.sql("select * from user_info")
  ...
  def calculateAndPersistAggrStat(spark: SparkSession, value: mutable.HashMap[String, Int], taskUUID: String): Unit = {
    // 从Accumulator统计串中获取值
